/**
 * payment.js - Payment page interaction logic
 * Includes API settlement and blockchain settlement features
 */
...
function updatePaymentDetailsUI(data) {
    ...
    if (refundButtonContainer) {
        if (data.status === 'paid' || data.status === 'confirmed') {
            ...
            refundButtonContainer.style.display = 'block';
            document.getElementById('refund-button').textContent = 'Check Refund Status';
            ...
            btn.textContent = 'Retrieve Funds';
            btn.onclick = () => handleEscrowRefund(data.id);
        } else {
    ...
    if (errorDetailsContainer) {
        if (data.status === 'failed' && data.errorDetails) {
            ...
            document.getElementById('error-message').textContent = errorDetails.message || 'Unknown error';
            document.getElementById('error-code').textContent = errorDetails.code || 'No error code';
...
async function connectWallet() {
  ...
  try {
    ...
    // 更新步骤状态
    document.getElementById('step1-status').textContent = 'Connected';
...
window.addEventListener('walletDisconnected', () => {
  ...
  document.getElementById('step1-status').textContent = 'Not Connected';
...
// 在此处新增：处理托管资金取回
async function handleEscrowRefund(paymentIntentId) {
    if (!confirm('确认取回托管资金？')) return;
    try {
        // 使用前端合约服务调用链上方法
        const tx = await window.contractService.cancelExpiredPayment(paymentIntentId);
