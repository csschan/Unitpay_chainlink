...
class SettlementService {
  ...
  async processSettlement(task) {
    console.log('[结算服务] 开始处理结算:', {
      id: task.paymentIntentId,
      status: task.status,
    ...
    try {
      ...
      // 更新支付意图状态为已结算
      // 获取当前状态历史
      const currentHistory = paymentIntent.statusHistory || [];
      
      // 从状态历史中找到最近的网络信息
      const lastNetworkInfo = [...currentHistory].reverse().find(entry => entry.network);
      const network = lastNetworkInfo?.network || process.env.NETWORK_TYPE || 'somnia';
