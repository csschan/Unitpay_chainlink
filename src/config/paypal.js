...
async function logPayPalError(subType, error, details = {}, options = {}) {
  // Log PayPal errors using the standard logger
  logger.error(`[PayPal/${subType}] ${error.message}`, { ...details, ...options, stack: error.stack });
}
...
async function logPayPalEvent(subType, details = {}, options = {}) {
  // Log PayPal events
  logger.info(`[PayPal Event/${subType}]`, { ...details, ...options });
}
...
async function logPayPalTransaction(subType, details = {}, options = {}) {
  ...
  logger.info(`[PayPal Transaction/${subType}]`, { ...details, ...options });
}
