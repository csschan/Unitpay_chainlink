...
const { PAYPAL_CLIENT_ID, PAYPAL_CLIENT_SECRET, PAYPAL_MODE, PAYPAL_MERCHANT_ID } = require('../config/paypal');
...
async function createPayPalClient() {
  try {
    ...
    console.log('Creating a new PayPal API client...');
    let environment;
...
exports.connectLPPayPal = async (req, res) => {
  try {
    ...
    // Validate that walletAddress and PayPal email are provided
    if (!walletAddress || !paypalEmail) {
      return res.status(400).json({
        success: false,
        message: 'Missing required parameters: walletAddress and PayPal email are required'
      });
    }
    ...
    try {
      // 检查LP是否有访问私钥
      const useBlockchain = req.body.useBlockchain === true;
      
      if (useBlockchain) {
        // 尝试在链上注册LP的PayPal邮箱
        const result = await contractService.registerLpPaypal(paypalEmail);
        
        await logPayPalEvent('lpConnectedOnchain', {
          lpId: lp.id,
          walletAddress,
          paypalEmail,
        ...
        return res.status(200).json({
          success: true,
          message: 'PayPal account connected and synced to blockchain successfully',
          data: {
            lpId: lp.id,
      ...
      console.error('Failed to register PayPal account on-chain:', blockchainError);
      await logPayPalError('connectLPPayPalOnchain', blockchainError);
      
      // 即使链上注册失败，仍然返回成功，因为数据库已更新
      return res.status(200).json({
        success: true,
        message: 'PayPal account connected successfully, but on-chain registration failed',
        data: {
          lpId: lp.id,
          walletAddress: lp.walletAddress,
          paypalEmail: paypalEmail,
    ...
    console.error('Failed to connect PayPal account:', error);
...
exports.createPayPalOrder = async (req, res) => {
  ...
  try {
    const { paymentIntentId, userWalletAddress, merchantPaypalEmail: requestMerchantEmail } = req.body;
    ...
    if (!paymentIntentId) {
      return res.status(400).json({
        success: false,
        message: 'Missing required parameter: paymentIntentId is required'
      });
    }
    ...
    console.log(`Found PaymentIntent, current status: ${paymentIntent.status}`);
    ...
    if (paymentIntent.status !== 'created' && paymentIntent.status !== 'processing' && paymentIntent.status !== 'claimed') {
      ...
      return res.status(400).json({
        success: false,
        message: `PaymentIntent status ${paymentIntent.status} does not allow order creation`
      });
    }
    ...
    if (requestMerchantEmail && 
      ...
      console.log(`Using merchant PayPal email from request: ${merchantPaypalEmail}`);
    }
    ...
    if (!merchantPaypalEmail && paymentIntent.merchantPaypalEmail) {
      ...
      console.log(`Using merchant PayPal email from PaymentIntent: ${merchantPaypalEmail}`);
    }
    ...
    if (!merchantPaypalEmail && paymentIntent.merchantInfo) {
      ...
      if (merchantInfo && merchantInfo.paypalEmail) {
        ...
        console.log(`Using merchant info PayPal email: ${merchantPaypalEmail}`);
      }
    }
    ...
    if (!merchantPaypalEmail && paymentIntent.lpWalletAddress) {
      try {
        ...
        if (lp && lp.paypalEmail) {
          ...
          console.log(`Using LP's PayPal email: ${merchantPaypalEmail}`);
        }
      } catch (error) {
        console.error('Failed to fetch LP info:', error);
      }
    }
    ...
    if (!merchantPaypalEmail || !merchantPaypalEmail.includes('@')) {
      ...
      return res.status(400).json({
        success: false,
        message: 'Unable to retrieve valid merchant PayPal email',
        error: 'No valid merchant PayPal email found, ensure merchant has set PayPal email'
      });
    }
    ...
    // 创建PayPal订单
    console.log(`Creating PayPal order via SDK, amount: ${paymentIntent.amount} ${paymentIntent.currency || 'USD'}`);
    ...
    request.requestBody({
      ...
      application_context: {
        return_url: `${process.env.APP_URL || 'http://localhost:4000'}/payment-success`,
        cancel_url: `${process.env.APP_URL || 'http://localhost:4000'}/payment-cancel`
      }
    });
    
    console.log('PayPal order request body:', JSON.stringify(request.requestBody));
    ...
    // 提交事务
    await t.commit();
    console.log(`Database update successful, transaction committed`);
    
    // 返回成功响应，包含PayPal商家ID以便SDK动态加载
    return res.status(200).json({
      success: true,
      ...
      data: {
        ...
        status: order.result.status,
        merchantId: order.result.purchase_units?.[0]?.payee?.merchant_id
      }
    });
...
exports.capturePayPalOrder = async (req, res) => {
  try {
    ...
    // 前端将使用 LP 钱包地址调用 submitOrderId 提交到区块链
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        paymentIntentId,
        paymentStatus,
        orderId: paypalOrderId || null,  // 用 orderId 代替 paypalOrderId
...
exports.refundPayPalPayment = async (req, res) => {
  try {
    ...
    // 创建退款请求，设置请求体（空对象为全额退款）
    const request = new paypal.payments.CapturesRefundRequest(captureId);
    const body = {};
    if (amount) {
      body.amount = { value: amount.toString(), currency_code: 'USD' };
    }
    if (reason) {
...
async function handlePaymentCaptureCompleted(webhookEvent) {
  try {
    ...
    if (paymentIntent.status !== 'confirmed') {
      ...
      // 合并 PayPal 捕获信息到 paymentProof
      let existingProof = paymentIntent.paymentProof || {};
      if (typeof existingProof === 'string') {
        try { existingProof = JSON.parse(existingProof); } catch (e) { existingProof = {}; }
      }
      if (!existingProof || typeof existingProof !== 'object') {
        existingProof = {};
      }
      existingProof.paypalCaptureId = captureId;
      // 更新支付意图，包括已合并的 paymentProof
      await paymentIntent.update({
        status: 'confirmed',
        statusHistory,
        processingDetails,
...
async function autoHandleExpiredOrders() {
  try {
    ...
    // 查找所有已过期但状态不是expired/cancelled/paid/confirmed/user_confirmed/settled的订单
    const expiredOrders = await PaymentIntent.findAll({
      where: {
        expiresAt: {
          [Op.lt]: new Date()
        },
        status: {
          [Op.notIn]: ['expired', 'cancelled', 'paid', 'confirmed', 'user_confirmed', 'settled']
        }
      }
    });
    ...
    for (const order of expiredOrders) {
      console.log(`处理过期订单: ${order.id}, 过期时间: ${order.expiresAt}`);
...
/**
 * 提交PayPal订单ID到区块链
 * @route POST /api/payment/paypal/submit-blockchain-order
 * @access Public
 */
exports.submitPaypalOrderToBlockchain = async (req, res) => {
  try {
    const { paymentIntentId, paypalOrderId, blockchainPaymentId, blockchainTxHash } = req.body;
    if (!paymentIntentId || !paypalOrderId || !blockchainPaymentId || !blockchainTxHash) {
      return res.status(400).json({ success: false, message: '缺少必要参数：paymentIntentId、paypalOrderId、blockchainPaymentId、blockchainTxHash 必须提供' });
    }
    
    const paymentIntent = await PaymentIntent.findOne({ where: { id: paymentIntentId } });
    if (!paymentIntent) {
      return res.status(404).json({ success: false, message: '未找到该支付意图' });
    }
    
    // 更新订单记录，存储链上提交信息
    await paymentIntent.update({ paypalOrderId, blockchainPaymentId, blockchainTxHash, lastVerifiedAt: new Date() });
      
      await logPayPalEvent('paypalOrderIdSubmittedToBlockchain', {
      paymentIntentId,
        paypalOrderId,
      blockchainPaymentId,
      ...
      // 更新 contractListener 映射
      if (global.contractListener) {
        console.log(`ContractListenerService: 添加 paymentIdHash 映射 for ${blockchainPaymentId}`);
        global.contractListener.addPaymentId(paymentIntent.id, blockchainPaymentId);
      }
      
      return res.status(200).json({
        success: true,
      message: '记录链上订单信息成功',
      data: { paymentIntentId, paypalOrderId, blockchainPaymentId, blockchainTxHash }
      });
  } catch (error) {
    console.error('记录链上PayPal订单信息失败:', error);
    await logPayPalError('submitPaypalOrderToBlockchain', error);
    return res.status(500).json({ success: false, message: '记录链上PayPal订单信息失败', error: process.env.NODE_ENV === 'development' ? error.message : '服务器内部错误' });
  }
};

/**
 * 获取区块链上的PayPal验证状态
 * @route GET /api/payment/paypal/blockchain-verification-status/:paymentIntentId
 * @access Public
 */
exports.getBlockchainVerificationStatus = async (req, res) => {
  try {
    const { paymentIntentId } = req.params;
    ...
    if (!paymentIntent.blockchainPaymentId) {
      return res.status(400).json({
        success: false,
        message: '该支付意图没有关联的区块链支付ID'
      });
    }
    
    // 从数据库读取最新状态
    const { status: currentStatus, statusHistory } = paymentIntent;
    return res.status(200).json({
      success: true,
      data: {
        paymentIntentId: paymentIntent.id,
    ...
    await logPayPalError('getBlockchainVerificationStatus', error);
    return res.status(500).json({
      success: false,
      message: '获取验证状态失败',
      error: process.env.NODE_ENV === 'development' ? error.message : '服务器内部错误'
...
module.exports = {
  ...
  // 提交PayPal订单ID到区块链
  submitPaypalOrderToBlockchain: exports.submitPaypalOrderToBlockchain,
  
  // 获取区块链上的PayPal验证状态
  getBlockchainVerificationStatus: exports.getBlockchainVerificationStatus
}; 