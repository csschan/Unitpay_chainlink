...
exports.getTaskPool = async (req, res) => {
  try {
    ...
    const lp = await LP.findOne({ 
      where: { 
        walletAddress: walletAddress.toLowerCase() // 转换为小写进行比较
      } 
    });
...
exports.getLPDirect = async (req, res) => {
  try {
    ...
    // 直接返回LP数据对象，不包装
    return res.status(200).json(lp.toJSON());
...
/**
 * 更新LP费率
 * @route PUT /api/lp/fee-rates
 * @access Public
 */
exports.updateFeeRates = async (req, res) => {
  try {
    const { walletAddress, feeRates } = req.body;
    
    // 验证请求数据
    if (!walletAddress || !feeRates) {
      return res.status(400).json({
        success: false,
        message: '缺少必要参数：钱包地址和费率设置必须提供'
      });
    }
    ...
    // 验证费率格式
    if (typeof feeRates !== 'object') {
      return res.status(400).json({
        success: false,
        message: '费率设置必须是一个对象，例如 {"PayPal": 0.5, "Alipay": 0.3}'
      });
    }
    
    // 检查费率值是否合法
    for (const platform in feeRates) {
      const rate = parseFloat(feeRates[platform]);
      if (isNaN(rate) || rate < 0 || rate > 10) {
        return res.status(400).json({
          success: false,
          message: `${platform}的费率设置无效，费率应为0-10之间的数字（代表百分比）`
        });
      }
    }
    ...
    return res.status(200).json({
      ...
      data: {
        lpId: lp.id,
        walletAddress: lp.walletAddress,
        feeRates: lp.feeRates
      }
    });
    
  } catch (error) {
    console.error('更新LP费率失败:', error);
    return res.status(500).json({
      success: false,
      message: '更新LP费率失败',
      error: process.env.NODE_ENV === 'development' ? error.message : '服务器内部错误'
